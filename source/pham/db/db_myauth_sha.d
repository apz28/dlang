...
protected:
    final ResultStatus getPasswordEx(scope const(char)[] userName, scope const(char)[] userPassword,
        bool leadingIndicator, ref CipherBuffer authData)
    {
        if (userPassword.length == 0)
            authData = CipherBuffer([0x00]);
        else
        {
            auto auth = calculateAuth(userName, userPassword, serverSalt);
            authData = leadingIndicator ? CipherBuffer([0x20]) : CipherBuffer();
            authData.put(auth[]);
        }
    ...
    static CipherBuffer xor(scope const(ubyte)[] left, scope const(ubyte)[] right) pure
    {
    ...
    static CipherBuffer xorNonce(scope const(ubyte)[] src, scope const(ubyte)[] nonce) pure
    {
        CipherBuffer result;
...
protected:
        ...
        scope const(ubyte)[] serverAuthData, ref CipherBuffer authData)
    {
        ...
        authData = CipherBuffer(output[0..outputLength]);
        return ResultStatus.ok();
    }
}
