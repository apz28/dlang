...
public:
	this(scope const(ubyte)[] key, scope const(ubyte)[] nonce) pure
    in
    {
        assert(key.length == keySize);
		assert(CipherKey.isValidKey(key));
        assert(nonce.length == nonceSize || nonce.length == nonceSizeX);
    }
    do
    {
		this(CipherKey(keySize * 8, key, nonce));
    }

	this(CipherKey cipherKey) pure
    in
    {
        assert(cipherKey.modulus.length == keySize);
		assert(CipherKey.isValidKey(cipherKey.modulus));
        assert(cipherKey.exponent.length == nonceSize || cipherKey.exponent.length == nonceSizeX);
    }
    do
    {
        this._parameters = CipherParameters(cipherKey);
        initKey();
	}
	...
	@property final uint counter() const @nogc pure
    {
		return _counter;
    }
...
private:
    ...
    {
        ubyte[keySize] initKey = void;
        ubyte[nonceSize] initNonce = void;

        // XChaCha20 uses the ChaCha20 core to mix 16 bytes of the nonce into a
		// derived key, allowing it to operate on a nonce of 24 bytes. See
		// draft-irtf-cfrg-xchacha-01, Section 2.3.
        if (_parameters.privateKey.exponent.length == nonceSizeX)
        {
		    initKey[] = hChaCha20(_parameters.privateKey.modulus, _parameters.privateKey.exponent[0..16]);
		    initNonce[0..4] = 0;
		    initNonce[4..12] = _parameters.privateKey.exponent[16..24];
        }
        else
        {
            initKey[] = _parameters.privateKey.modulus[];
            initNonce[] = _parameters.privateKey.exponent[];
        }

		this.key[0] = bytesToNative!uint(initKey[0..4]);
		this.key[1] = bytesToNative!uint(initKey[4..8]);
		this.key[2] = bytesToNative!uint(initKey[8..12]);
		this.key[3] = bytesToNative!uint(initKey[12..16]);
		this.key[4] = bytesToNative!uint(initKey[16..20]);
		this.key[5] = bytesToNative!uint(initKey[20..24]);
		this.key[6] = bytesToNative!uint(initKey[24..28]);
		this.key[7] = bytesToNative!uint(initKey[28..32]);

	    this.nonce[0] = bytesToNative!uint(initNonce[0..4]);
		this.nonce[1] = bytesToNative!uint(initNonce[4..8]);
		this.nonce[2] = bytesToNative!uint(initNonce[8..12]);
    }
    ...
    do
    {
		enum overflowSize = 1UL << 32;

		const resultLength = input.length;
		auto resultOutput = output;

		// First, drain any remaining key stream from a previous XORKeyStream.
		if (leftLen != 0)
		{
			auto keyStream = leftBuf[blockSize - leftLen..$];
			if (input.length < keyStream.length)
				keyStream = keyStream[0..input.length];

			foreach (i, b; keyStream)
				output[i] = input[i] ^ b;

			leftLen -= keyStream.length;
			output = output[keyStream.length..$];
            input = input[keyStream.length..$];
		}
		...
		const numBlocks = (input.length + blockSize - 1) / blockSize;
		const overBlocks = cast(ulong)_counter + numBlocks;
		if (_overflow || overBlocks > overflowSize)
			return null;
        else if (overBlocks == overflowSize)
			_overflow = true;
		...
		const fullBlockLength = input.length - (input.length % blockSize);
		if (fullBlockLength != 0)
        {
			if (!processBlock(output[0..fullBlockLength], input[0..fullBlockLength]))
				return null;
        }
		input = input[fullBlockLength..$];
		output = output[fullBlockLength..$];

		// If we have a partial (multi-)block, pad it for xorKeyStreamBlocks, and
		// keep the leftover keystream for the next XORKeyStream invocation.
		if (input.length != 0)
		{
			leftBuf[input.length..$] = 0;
			leftBuf[0..input.length] = input[0..$];
			if (!processBlock(leftBuf[], leftBuf[]))
				return null;
			output[0..input.length] = leftBuf[0..input.length];
			leftLen = blockSize - input.length;
		}

        return resultOutput[0..resultLength];
    }

    final bool processBlock(ubyte[] dst, scope const(ubyte)[] src) @nogc pure
    in
    {
        assert(dst.length == src.length);
        assert(dst.length % blockSize == 0);
    }
	do
    {
		...
		uint c0 = j0, c1 = j1, c2 = j2, c3 = j3;
		uint c4 = key[0], c5 = key[1], c6 = key[2], c7 = key[3];
		uint c8 = key[4], c9 = key[5], c10 = key[6], c11 = key[7];
		uint preCounter = _counter, c13 = nonce[0], c14 = nonce[1], c15 = nonce[2];
		...
		if (!precompDone)
        {
			quarterRound(c1, c5, c9, c13, p1, p5, p9, p13);
			quarterRound(c2, c6, c10, c14, p2, p6, p10, p14);
			quarterRound(c3, c7, c11, c15, p3, p7, p11, p15);
			precompDone = true;
		}

		while (dst.length != 0 && src.length != 0)
        {
			// The remainder of the first column round.
			uint fcr0 = void, fcr4 = void, fcr8 = void, fcr12 = void;
            quarterRound(c0, c4, c8, _counter, fcr0, fcr4, fcr8, fcr12);

			// The second diagonal round.
			uint x0 = void, x5 = void, x10 = void, x15 = void;
            quarterRound(fcr0, p5, p10, p15, x0, x5, x10, x15);
			uint x1 = void, x6 = void, x11 = void, x12 = void;
            quarterRound(p1, p6, p11, fcr12, x1, x6, x11, x12);
			uint x2 = void, x7 = void, x8 = void, x13 = void;
            quarterRound(p2, p7, fcr8, p13, x2, x7, x8, x13);
			uint x3 = void, x4 = void, x9 = void, x14 = void;
            quarterRound(p3, fcr4, p9, p14, x3, x4, x9, x14);

			// The remaining 18 rounds.
			foreach (i; 0..9)
            {
				// Column round.
				quarterRound(x0, x4, x8, x12, x0, x4, x8, x12);
				quarterRound(x1, x5, x9, x13, x1, x5, x9, x13);
				quarterRound(x2, x6, x10, x14, x2, x6, x10, x14);
				quarterRound(x3, x7, x11, x15, x3, x7, x11, x15);

				// Diagonal round.
				quarterRound(x0, x5, x10, x15, x0, x5, x10, x15);
				quarterRound(x1, x6, x11, x12, x1, x6, x11, x12);
				quarterRound(x2, x7, x8, x13, x2, x7, x8, x13);
				quarterRound(x3, x4, x9, x14, x3, x4, x9, x14);
			}
			...
			addXOr(dst[48..52], src[48..52], x12, _counter);
			addXOr(dst[52..56], src[52..56], x13, c13);
			addXOr(dst[56..60], src[56..60], x14, c14);
			addXOr(dst[60..64], src[60..64], x15, c15);

			if (!setCounter(_counter + 1))
				return false;
	...
	// counter value.
	final bool setCounter(const(uint) newCounter) @nogc pure
    {
		// Internally, s may buffer multiple blocks, which complicates this
		// implementation slightly. When checking whether the counter has rolled
		// back, we must use both s.counter and s.len to determine how many blocks
		// we have already output.
		const outputCounter = _counter - (leftLen / blockSize);
		// setCounter attempted to rollback counter?
		if (_overflow || newCounter < outputCounter)
			return false;

		// In the general case, we set the new counter value and reset leftLen to 0,
		// causing the next call to XORKeyStream to refill the buffer. However, if
		// we're advancing within the existing buffer, we can save work by simply
		// setting leftLen.
		if (newCounter < _counter)
        {
			leftLen = (_counter - newCounter) * blockSize;
        }
		else
        {
			_counter = newCounter;
	...
	static void addXOr(ubyte[] dst, scope const(ubyte)[] src, uint a, uint b) @nogc pure
    {
		a += b;
		dst[0] = src[0] ^ cast(ubyte)(a);
		dst[1] = src[1] ^ cast(ubyte)(a >> 8);
		dst[2] = src[2] ^ cast(ubyte)(a >> 16);
		dst[3] = src[3] ^ cast(ubyte)(a >> 24);
	}

    static ubyte[keySize] hChaCha20(scope const(ubyte)[] key, scope const(ubyte)[] nonce) @nogc pure
    in
    {
        assert(key.length >= keySize);
    ...
    do
    {
	    ...
	    uint x4 = bytesToNative!uint(key[0..4]);
	    uint x5 = bytesToNative!uint(key[4..8]);
	    uint x6 = bytesToNative!uint(key[8..12]);
	    uint x7 = bytesToNative!uint(key[12..16]);
	    uint x8 = bytesToNative!uint(key[16..20]);
	    uint x9 = bytesToNative!uint(key[20..24]);
	    uint x10 = bytesToNative!uint(key[24..28]);
	    uint x11 = bytesToNative!uint(key[28..32]);

	    uint x12 = bytesToNative!uint(nonce[0..4]);
	    uint x13 = bytesToNative!uint(nonce[4..8]);
	    uint x14 = bytesToNative!uint(nonce[8..12]);
	    uint x15 = bytesToNative!uint(nonce[12..16]);
        ...
        ubyte[keySize] result = void;
	    result[0..4] = nativeToBytes(x0);
	    result[4..8] = nativeToBytes(x1);
	    result[8..12] = nativeToBytes(x2);
	    result[12..16] = nativeToBytes(x3);
	    result[16..20] = nativeToBytes(x12);
	    result[20..24] = nativeToBytes(x13);
	    result[24..28] = nativeToBytes(x14);
	    result[28..32] = nativeToBytes(x15);

        return result;
    }

    static void quarterRound(uint a, uint b, uint c, uint d,
        ref uint a2, ref uint b2, ref uint c2, ref uint d2) @nogc pure
    {
	    a += b;
	    d ^= a;
	    d = rol(d, 16);
...
unittest // CipherChaCha20.quarterRound
{
    import pham.utl.test;
    traceUnitTest!("pham.cp")("unittest pham.cp.cipher_chacha.CipherChaCha20.quarterRound");

	uint a = 0x1111_1111, b = 0x0102_0304, c = 0x9b8d_6f43, d = 0x0123_4567;
	uint a2, b2, c2, d2;
	CipherChaCha20.quarterRound(a, b, c, d, a2, b2, c2, d2);
	assert(a2 == 0xea2a_92f4, a2.dgToHex());
	assert(b2 == 0xcb1c_f8ce, b2.dgToHex());
	assert(c2 == 0x4581_472e, c2.dgToHex());
	assert(d2 == 0x5881_c4bb, d2.dgToHex());
}

unittest // CipherChaCha20.hChaCha20
{
    import pham.utl.object : bytesFromHexs;
    import pham.utl.test;
    traceUnitTest!("pham.cp")("unittest pham.cp.cipher_chacha.CipherChaCha20.hChaCha20");
	...
	auto v = CipherChaCha20.hChaCha20(key, nonce);
	assert(v[] == expected);
}
...
unittest // CipherChaCha20
{
	...
	static void test(TestCase testCase, in int fromLine = __LINE__)
    {
        auto cipher = new CipherChaCha20(bytesFromHexs(testCase.key), bytesFromHexs(testCase.nonce));
