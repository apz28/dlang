...
*/
bool isLoggingEnabled(LogLevel ll, LogLevel loggerLL, LogLevel globalLL) nothrow @safe
{
    switch (ll)
    {
        case LogLevel.trace:
...
private string parentOf(string mod) nothrow @safe
{
...
*/
void systimeToISOString(OutputRange)(OutputRange o, const ref SysTime time) @safe
...
*/
version (none)
void log(string moduleName = __MODULE__, A...)(const LogLevel ll, lazy bool condition, lazy A args,
    int line = __LINE__, string file = __FILE__,
    string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
if (args.length != 1)
{
    static if (isLoggingActive)
    {
        ...
        {
            stdThreadLocalLog.log!(moduleName)(ll, condition, args, line, file, funcName, prettyFuncName);
        }
    }
}

/// Ditto
void log(T, string moduleName = __MODULE__)(const LogLevel ll, lazy bool condition, lazy T arg,
    int line = __LINE__, string file = __FILE__,
    string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
{
    static if (isLoggingActive)
...
*/
version (none)
void log(string moduleName = __MODULE__, A...)(const LogLevel ll, lazy A args,
    int line = __LINE__, string file = __FILE__,
    string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
if (args.length > 1 && !is(Unqual!(A[0]) : bool))
{
    static if (isLoggingActive)
    {
        if (ll >= moduleLogLevel!moduleName)
        {
            stdThreadLocalLog.log!(moduleName)(ll, args, line, file, funcName, prettyFuncName);
        }
    }
}
    ...
    string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
{
    static if (isLoggingActive)
    {
        if (ll >= moduleLogLevel!moduleName)
        {
            stdThreadLocalLog.log!(T, moduleName)(ll, arg, line, file, funcName, prettyFuncName);
        }
    }
}
...
*/
version (none)
void log(string moduleName = __MODULE__, A...)(lazy bool condition, lazy A args,
    int line = __LINE__, string file = __FILE__,
    string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
if (args.length != 1)
{
    static if (isLoggingActive)
    {
        stdThreadLocalLog.log!(moduleName)(stdThreadLocalLog.logLevel, condition, args, line, file, funcName, prettyFuncName);
    }
}
    ...
    string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
{
    static if (isLoggingActive)
    {
        stdThreadLocalLog.log!(T, moduleName)(stdThreadLocalLog.logLevel, condition, arg, line, file, funcName, prettyFuncName);
    }
}
...
*/
version (none)
void log(string moduleName = __MODULE__, A...)(lazy A args,
    int line = __LINE__, string file = __FILE__,
    string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
if ((args.length > 1 && !is(Unqual!(A[0]) : bool) && !is(Unqual!(A[0]) == LogLevel))
    || args.length == 0)
{
    static if (isLoggingActive)
    {
        stdThreadLocalLog.log!(moduleName)(stdThreadLocalLog.logLevel, args, line, file, funcName, prettyFuncName);
    }
}

void log(T, string moduleName = __MODULE__)(lazy T arg,
    int line = __LINE__, string file = __FILE__,
    string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
{
    static if (isLoggingActive)
    {
        stdThreadLocalLog.log!(T, moduleName)(stdThreadLocalLog.logLevel, arg, line, file, funcName, prettyFuncName);
    }
}
...
--------------------
*/
void logf(string moduleName = __MODULE__, A...)(const LogLevel ll, lazy bool condition, lazy string msg, lazy A args,
    int line = __LINE__, string file = __FILE__,
    string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
{
    static if (isLoggingActive)
    {
        if (ll >= moduleLogLevel!moduleName)
        {
            stdThreadLocalLog.logf!(moduleName)(ll, condition, msg, args, line, file, funcName, prettyFuncName);
        }
    }
}
...
--------------------
*/
void logf(string moduleName = __MODULE__, A...)(const LogLevel ll, lazy string msg, lazy A args,
    int line = __LINE__, string file = __FILE__,
    string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
{
    static if (isLoggingActive)
    {
        if (ll >= moduleLogLevel!moduleName)
        {
            stdThreadLocalLog.logf!(moduleName)(ll, msg, args, line, file, funcName, prettyFuncName);
        }
    }
}
...
--------------------
*/
void logf(string moduleName = __MODULE__, A...)(lazy bool condition, lazy string msg, lazy A args,
    int line = __LINE__, string file = __FILE__,
    string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
{
    static if (isLoggingActive)
    {
        stdThreadLocalLog.logf!(moduleName)(stdThreadLocalLog.logLevel, condition, msg, args, line, file, funcName, prettyFuncName);
    }
}
...
--------------------
*/
void logf(string moduleName = __MODULE__, A...)(lazy string msg, lazy A args,
    int line = __LINE__, string file = __FILE__,
    string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
{
    static if (isLoggingActive)
    {
        stdThreadLocalLog.logf!(moduleName)(stdThreadLocalLog.logLevel, msg, args, line, file, funcName, prettyFuncName);
    }
}
...
{
    void defaultLogFunction(string moduleName = __MODULE__, A...)(lazy A args,
        int line = __LINE__, string file = __FILE__,
        string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
    if ((args.length > 0 && !is(Unqual!(A[0]) : bool)) || args.length == 0)
    {
        ...
        {
            stdThreadLocalLog.memLogFunctions!(ll).logImpl!(moduleName)(args, line, file, funcName, prettyFuncName);
        }
    }

    void defaultLogFunction(string moduleName = __MODULE__, A...)(lazy bool condition, lazy A args,
        int line = __LINE__, string file = __FILE__,
        string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
    {
        static if (isLoggingActiveAt!ll && ll >= moduleLogLevel!moduleName)
        {
            stdThreadLocalLog.memLogFunctions!(ll).logImpl!(moduleName)(condition, args, line, file, funcName, prettyFuncName);
        }
    }
}
...
{
    void defaultLogFunctionf(string moduleName = __MODULE__, A...)(lazy string msg, lazy A args,
        int line = __LINE__, string file = __FILE__,
        string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
    {
        static if (isLoggingActiveAt!ll && ll >= moduleLogLevel!moduleName)
        {
            stdThreadLocalLog.memLogFunctions!(ll).logImplf!(moduleName)(msg, args, line, file, funcName, prettyFuncName);
        }
    }

    void defaultLogFunctionf(string moduleName = __MODULE__, A...)(lazy bool condition, lazy string msg, lazy A args,
        int line = __LINE__, string file = __FILE__,
        string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
    {
        static if (isLoggingActiveAt!ll && ll >= moduleLogLevel!moduleName)
        {
            stdThreadLocalLog.memLogFunctions!(ll).logImplf!(moduleName)(condition, msg, args, line, file, funcName, prettyFuncName);
        }
    }
}
...
private struct MsgRange
{
    ...
    void put(dchar elem) @safe
    {
        ...
        char[4] buffer;
        const len = encode(buffer, elem);
        log.logMsgPart(buffer[0 .. len]);
    }
}

private void formatString(A...)(MsgRange oRange, A args) nothrow @trusted
{
    try
    {
        foreach (arg; args)
...
enum LogLevel : ubyte
{
    ...
    warning = 96, /** warnings about the program should be displayed with this level. */
...
abstract class Logger
{
    ...
    */
    this(const LogLevel lv) nothrow @safe
    {
        this.logLevel_ = lv;
        this.fatalHandler_ = delegate() { throw new Error("A fatal log message was logged"); };
    ...
    */
    ...
    ----------------
    */
    protected void beginLogMsg(int line, string file, string funcName, string prettyFuncName, string moduleName,
        LogLevel logLevel, Tid threadId, SysTime timestamp, Logger logger) nothrow @safe
    {
        static if (isLoggingActive)
        {
            msgAppender = appender!string();
            header = LogEntry(line, file, funcName, prettyFuncName, moduleName, logLevel, threadId, timestamp, null, logger);
        }
    }
    ...
    */
    ...
    -----------
    */
    @property final LogLevel logLevel() const nothrow pure @safe @nogc
    {
        return trustedLoad(this.logLevel_);
    }

    /// Ditto
    @property final void logLevel(const LogLevel lv) nothrow @safe @nogc
    {
    ...
    */
    @property final void delegate() fatalHandler() nothrow @safe @nogc
    {
        scope (failure) assert(0);
    ...
    /// Ditto
    @property final void fatalHandler(void delegate() @safe fh) nothrow @safe @nogc
    {
        scope (failure) assert(0);
    ...
    */
    template memLogFunctions(LogLevel ll)
    {
        ...
        --------------------
        */
        void logImpl(string moduleName = __MODULE__, A...)(lazy A args,
            int line = __LINE__, string file = __FILE__,
            string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
        if (args.length == 0 || (args.length > 0 && !is(A[0] : bool)))
        {
            ...
            try
            {
                synchronized (mutex)
                {
                    ...
                    {
                        this.beginLogMsg(line, file, funcName, prettyFuncName, moduleName, ll, thisTid, Clock.currTime, this);
        ...
        --------------------
        */
        void logImpl(string moduleName = __MODULE__, A...)(lazy bool condition, lazy A args,
            int line = __LINE__, string file = __FILE__,
            string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
        {
            static if (isLoggingActiveAt!ll && ll >= moduleLogLevel!moduleName)
        ...
        --------------------
        */
        void logImplf(string moduleName = __MODULE__, A...)(lazy bool condition, lazy string msg, lazy A args,
            int line = __LINE__, string file = __FILE__,
            string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
        {
            static if (isLoggingActiveAt!ll && ll >= moduleLogLevel!moduleName)
        ...
        --------------------
        */
        void logImplf(string moduleName = __MODULE__, A...)(lazy string msg, lazy A args,
            int line = __LINE__, string file = __FILE__,
            string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
        {
            static if (isLoggingActiveAt!ll && ll >= moduleLogLevel!moduleName)
    ...
    */
    ...
    if (args.length > 1)
    {
        ...
        try
        {
            synchronized (mutex)
            {
                if (isLoggingEnabled(ll, this.logLevel_, globalLogLevel) && ll >= moduleLogLevel!moduleName && condition)
                {
                    this.beginLogMsg(line, file, funcName, prettyFuncName, moduleName, ll, thisTid, Clock.currTime, this);
    ...
    /// Ditto
    void log(T, string moduleName = __MODULE__)(const LogLevel ll, lazy bool condition, lazy T args,
        int line = __LINE__, string file = __FILE__,
        string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
    {
        static if (isLoggingActive)
    ...
    */
    ...
    if ((args.length > 1 && !is(Unqual!(A[0]) : bool)) || args.length == 0)
    {
        ...
        try
        {
            synchronized (mutex)
            {
                if (isLoggingEnabled(ll, this.logLevel_, globalLogLevel) && ll >= moduleLogLevel!moduleName)
                {
                    this.beginLogMsg(line, file, funcName, prettyFuncName, moduleName, ll, thisTid, Clock.currTime, this);
    ...
    /// Ditto
    void log(T, string moduleName = __MODULE__)(const LogLevel ll, lazy T args,
        int line = __LINE__, string file = __FILE__,
        string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
    {
        static if (isLoggingActive)
    ...
    */
    ...
    if (args.length > 1)
    {
        ...
        try
        {
            synchronized (mutex)
            {
                if (isLoggingEnabled(this.logLevel_, this.logLevel_, globalLogLevel) && condition)
                {
                    this.beginLogMsg(line, file, funcName, prettyFuncName, moduleName, this.logLevel_, thisTid, Clock.currTime, this);
    ...
    /// Ditto
    void log(T, string moduleName = __MODULE__)(lazy bool condition, lazy T args,
        int line = __LINE__, string file = __FILE__,
        string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
    {
        static if (isLoggingActive)
    ...
    */
    ...
    void log(string moduleName = __MODULE__, A...)(lazy A args,
        int line = __LINE__, string file = __FILE__,
        string funcName = __FUNCTION__, string prettyFuncName = __PRETTY_FUNCTION__) nothrow
    if ((args.length > 1 && !is(Unqual!(A[0]) : bool) && !is(immutable A[0] == immutable LogLevel))
        || args.length == 0)
    {
        static if (isLoggingActive)
        try
 ...
 */
private @property Logger defaultSharedLoggerImpl() nothrow @trusted
{
...
-------------
*/
@property Logger sharedLog() nothrow @safe
{
    static auto trustedLoad(ref shared Logger logger) nothrow @trusted
    {
...
*/
private @property Logger stdThreadLocalLogImpl() nothrow @trusted
{
    import std.conv : emplace;
...
functions.
*/
@property Logger stdThreadLocalLog() nothrow @safe
{
    // If we have set up our own logger use that
...
/// Ditto
@property void stdThreadLocalLog(Logger logger) nothrow @safe
{
...
@safe unittest
{
    ...
    assert(l.logLevel == LogLevel.all);
    sharedLog = l;
    assert(globalLogLevel == LogLevel.all, to!string(globalLogLevel));
                        ...
                        foreach (prntf; [true, false])
                        {
                                    ...
                                    LogLevel.fatal, LogLevel.off])
                            {
                                foreach (singleMulti; 0 .. 2)
                                {
                                    ...
                                    if (memOrG)
                                    {
                                        if (prntf)
                                        {
                                            if (cond)
                                            {
                                                if (singleMulti == 0)
                                                {
                                                    mem.logf(ll2, condValue, "%s", value);
                                                    lineCall = __LINE__;
                                                }
                                                else
                                                {
                                                    mem.logf(ll2, condValue, "%d %d", value, value);
                                                    lineCall = __LINE__;
                                                }
                                            }
                                            else
                                        ...
                                        else
                                        {
                                            if (cond)
                                            {
                                                ...
                                                {
                                                    mem.log(ll2, condValue, to!string(value), value);
                                                    lineCall = __LINE__;
                                                }
                                            }
                                            else
                                            {
                                                ...
                                                {
                                                    mem.log(ll2, to!string(value), value);
                                                    lineCall = __LINE__;
                                                }
                                            }
                                        }
                                    ...
                                    else
                                    {
                                        if (prntf)
                                        {
                                            if (cond)
                                            {
                                                ...
                                                {
                                                    logf(ll2, condValue, "%s %d", value, value);
                                                    lineCall = __LINE__;
                                                }
                                            }
                                            else
                                        ...
                                        else
                                        {
                                            if (cond)
                                            {
                                                ...
                                                {
                                                    log(ll2, condValue, value, to!string(value));
                                                    lineCall = __LINE__;
                                                }
                                            }
                                            else
                                    ...
                                    bool shouldLog = ll2Off && gllOff && llOff && condFalse && ll2VSgll && ll2VSll;
                        ...
                        foreach (memOrG; [true, false])
                        {
                            foreach (prntf; [true, false])
                            {
                                foreach (singleMulti; 0 .. 2)
                                {
                                    ...
                                    if (memOrG)
                                    {
                                        if (prntf)
                                        {
                                            if (cond)
                                            {
                                                if (singleMulti == 0)
                                                {
                                                    mem.logf(condValue, "%s", value);
                                                    lineCall = __LINE__;
                                                }
                                                else
                                                {
                                                    mem.logf(condValue, "%d %d", value, value);
                                                    lineCall = __LINE__;
                                                }
                                            }
                                            else
                                        ...
                                        else
                                        {
                                            if (cond)
                                            {
                                                if (singleMulti == 0)
                                                {
                                                    mem.log(condValue, to!string(value));
                                                    lineCall = __LINE__;
                                                }
                                                else
                                                {
                                                    mem.log(condValue, to!string(value), value);
                                                    lineCall = __LINE__;
                                                }
                                            }
                                            else
                                    ...
                                    else
                                    {
                                        if (prntf)
                                        {
                                            if (cond)
                                            {
                                                ...
                                                {
                                                    logf(condValue, "%s %d", value, value);
                                                    lineCall = __LINE__;
                                                }
                                            }
                                            else
                                        ...
                                        else
                                        {
                                            if (cond)
                                            {
                                                if (singleMulti == 0)
                                                {
                                                    log(condValue, to!string(value));
                                                    lineCall = __LINE__;
                                                }
                                                else
                                                {
                                                    log(condValue, value, to!string(value));
                                                    lineCall = __LINE__;
                                                }
                                            }
                                            else
                                            {
                                                ...
                                                {
                                                    log(true, value, to!string(value)); //true=BUG
                                                    lineCall = __LINE__;
                                                }
                                            }
                                        }
                                    ...
                                    bool llVSgll = (ll >= gll);
                                    bool tllVSll = (stdThreadLocalLog.logLevel >= ll);
                                    bool condFalse = (cond ? condValue : true);
...
private auto trustedLoad(T)(ref shared T value) nothrow @trusted
{
    import core.atomic : atomicLoad, MemoryOrder;
...
// ditto
private void trustedStore(T)(ref shared T dst, ref T src) nothrow @trusted
{
    import core.atomic : atomicStore, MemoryOrder;
